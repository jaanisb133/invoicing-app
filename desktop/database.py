"""
Database module for Veggie Invoice Manager.
Handles SQLite database creation, migrations, and operations.
"""
...
def get_db_path():
    """Get the database path in the same directory as the executable/script."""
...
def get_next_doc_number(doc_type, year, conn=None):
    ...
    Prefix comes from settings (buy_doc_prefix / sell_doc_prefix).
    """
    close_conn = False
    if conn is None:
        conn = get_connection()
    ...
    # Check for recycled numbers first
...
def create_document(doc_type, client_id, doc_date, items, vat_rate=21.0, notes=""):
    ...
    conn = get_connection()
    try:
        # Check stock for sell documents
        if doc_type == "sell":
            for item in items:
...
def delete_document(doc_id):
    ...
    doc = conn.execute("SELECT * FROM documents WHERE id = ?", (doc_id,)).fetchone()
    if doc:
        # Extract the sequence number from doc_number (format: PREFIX-YEAR-0001)
        parts = doc["doc_number"].rsplit("-", 1)
        if len(parts) == 2:
            try:
                seq_num = int(parts[1])
                year = int(doc["doc_date"][:4])
                # Add to recycled numbers
                conn.execute(
